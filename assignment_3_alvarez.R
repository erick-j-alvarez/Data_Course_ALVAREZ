...
# view the 'iris' data frame 
View(iris)

# function to subset rows using their position (unlike filter() that does so based on logical operations)
?slice()

# uses the slice() function to filter only the even numbered rows from the 'iris' data frame
even_iris_dat <- iris |>
  slice(seq(2, 150, 2))

# or use the filter() function . . . 
even_iris_dat <- iris |>
  filter(row_number() %% 2 == 0) # uses modulo in order to logically assess a row being an even number by dividing by 2 and returning the remainder



# 2.  Create a new object called iris_chr which is a copy of iris, except where every column is a character class

# creates a copy of 'even_iris_dat' that uses mutate_all() in order to apply the chr data type to all columns
iris_chr <- even_iris_dat |>
  mutate_all(as.character)

# check the data structure of the iris_chr object 
str(iris_chr)
...
# Sepal.Area is created by multiplying Sepal.Length and Sepal.Width but we first have to convert those data types to num
Sepal.Area <- as.numeric(iris_chr$Sepal.Length) * as.numeric(iris_chr$Sepal.Width)



# 4.  Add Sepal.Area to the iris data frame as a new column

# adds Sepal.Area to our iris_chr data frame
sepal_area_df <- iris_chr |>
  mutate(Sepal.Area = Sepal.Area) |>
  relocate(Sepal.Area, .after = Sepal.Width) # moves the Sepal.Area column to the right of the Sepal.Width column

# check the column names and data types found in sepal_area_df
str(sepal_area_df)
...
# uses filter() to filter rows with a Sepal.Area value greater than 20 
big_area_iris <- sepal_area_df |>
